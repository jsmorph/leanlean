...
| Real exponential-series summability | `Mathlib.Analysis.SpecificLimits.Normed` | `Real.summable_pow_div_factorial` | Accepted | Restored-checkout v4 cache replay reused 11,483 declaration entries, checked 2,181 new entries, accepted 13,664 target declarations, and produced environment size 14,701.  The artifact has 1,112,052 NDJSON rows and a 57 MB file size; the target theorem checked in 285 ms, and the slowest checked dependency was `Real.exists_isLUB` at 9,326 ms. |
...
| Polynomial constant multiplication | `Mathlib.Algebra.Polynomial.Basic` | `Polynomial.C_mul` | Accepted | The first run exposed the `Eq.ndrec` boundary: MPC had modeled Lean's transparent `Eq.ndrec` abbreviation as a second equality primitive, so conversion could leave `Eq.rec` and `Eq.ndrec` as distinct constant heads.  Under the current checker and v4 cache, replay reused 4,034 declaration entries, checked 256 new entries, accepted 4,290 target declarations, and produced environment size 4,795. |
...
| Polynomial `X` multiplication | `Mathlib.Algebra.Polynomial.Basic` | `Polynomial.X_mul` | Accepted | Current v4 cache replay reused 4,238 declaration entries, checked 35 new entries, accepted 4,273 target declarations, and produced environment size 4,760. |
...
| Tensor-product exactness symmetry | `Mathlib.LinearAlgebra.TensorProduct.RightExactness` | `LinearMap.rTensor_exact_iff_lTensor_exact` | Accepted | Restored-checkout v4 cache replay reused 1,646 declaration entries, checked 288 new entries, accepted 1,934 target declarations, and produced environment size 2,340.  The target theorem checked in 403 ms; the slowest checked dependencies were `TensorProduct.uncurry._proof_2` at 24,818 ms, `TensorProduct.uncurry._proof_1` at 16,257 ms, and `TensorProduct.lift.equiv._proof_2` at 12,196 ms. |
...
| Abelian image/coimage batch | `Mathlib.CategoryTheory.Abelian.Basic` | Five roots through `CategoryTheory.Abelian.monoLift_comp` | Accepted | Current v4 cache replay reused 5,484 declaration entries, checked 68 new entries, accepted 5,552 target declarations, and produced environment size 6,182.  The batch widens the abelian image/coimage comparison and epi/mono factorization coverage. |
| Abelian pullback/pushout batch | `Mathlib.CategoryTheory.Abelian.Basic` | Six roots through `CategoryTheory.Abelian.mono_inl_of_factor_thru_epi_mono_factorization` | Accepted | Current v4 cache replay reused 5,584 declaration entries, checked 48 new entries, accepted 5,632 target declarations, and produced environment size 6,250.  The run checks pullback preservation of epimorphisms, pushout preservation of monomorphisms, and the biproduct support used in those proofs. |
| Abelian exactness batch | `Mathlib.CategoryTheory.Abelian.Basic` | Eight roots through `CategoryTheory.Functor.reflects_exact_of_faithful` | Accepted | Current v4 cache replay reused 6,091 declaration entries, checked 342 new entries, accepted 6,433 target declarations, and produced environment size 7,115.  The batch first exposed the missing singleton-inductive conversion rule through `PUnit`, then accepted through exactness, left and right homology data, and faithful-functor exactness reflection. |
...
| Snake delta isomorphism | `Mathlib.Algebra.Homology.ShortComplex.SnakeLemma` | `CategoryTheory.ShortComplex.SnakeInput.δIso` plus support roots | Accepted | V4 cache replay reused 7,645 declaration entries, checked the final two entries, accepted 7,647 target declarations, and produced environment size 8,401.  The checked suffix took 61 ms. |
...
The real exponential-series summability probe accepts in the restored mathlib checkout.  `Real.summable_pow_div_factorial` exercises normed-series infrastructure, factorial and power arithmetic, and real completeness support after the earlier real-square-root and measure-theory probes had already populated part of that dependency region.  The result broadens analysis coverage without exposing a new rule package, although `Real.exists_isLUB` remains a visible dependency cost in cold checked suffixes.
...
The tensor-product exactness probe accepts and expands linear-algebra coverage beyond finite matrices and determinants.  `LinearMap.rTensor_exact_iff_lTensor_exact` stresses quotient-backed tensor-product definitions, bilinear lift support, and exactness statements over linear maps.  The selected theorem itself is cheap after the prefix, but `TensorProduct.uncurry` and lift-equivalence support proofs remain visible conversion-throughput costs.
...
The linear-map determinant probe accepts, but it remains the clearest example of replay-level conversion reuse paying for itself.  `LinearEquiv.noConfusion` and `_private.Mathlib.LinearAlgebra.InvariantBasisNumber.0.inducedEquiv._proof_4` both looked like generated-support or algebra-instance walls during diagnosis, but profiling classified the cost as repeated successful conversion rather than a missing MPC rule.  The accepted run uses the adapter success cache on checked misses, so this result does not move memoization or generated-declaration shortcuts into the trusted checker.
...
The delta-isomorphism snake probe also accepts.  It reuses the support prefix built by the earlier snake batches and checks only the final `isIso_δ` and `δIso` declarations.  This gives a cheap confirmation that the accepted snake support is reusable across a nearby root set.
