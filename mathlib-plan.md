...
| Linear-map determinant composition | `Mathlib.LinearAlgebra.Determinant` | `LinearMap.det_comp` | Rejected: performance | Cache stats replay reached declaration index 7358, `LinearEquiv.noConfusion`, before the determinant theorem.  The v4 retry reached the same started row with 4,118 reused declarations, 3,240 checked declarations, no stderr, and a 125 MB cache, then was interrupted for diagnosis.  Cached declaration profiling now reaches the selected declaration in about 78 seconds and reports no recursor, projection, quotient, equality-rec, equality-ndrec, or primitive-Nat head applications. |
