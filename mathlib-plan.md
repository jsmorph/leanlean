...
| Linear-map determinant composition | `Mathlib.LinearAlgebra.Determinant` | `LinearMap.det_comp` | Rejected: performance | Cache stats replay reached declaration index 7358, `LinearEquiv.noConfusion`, before the determinant theorem.  The v4 retry reached the same started row with 4,118 reused declarations, 3,240 checked declarations, no stderr, and a 125 MB cache, then was interrupted for diagnosis.  The hard declaration is generated structure support with static heads dominated by `Semiring.toNonAssocSemiring`, `HEq`, `LinearEquiv`, `RingHomInvPair`, `RingHom`, and `Module`. |
...
The linear-map determinant probe hits a generated-support performance boundary before the determinant composition theorem.  `LinearEquiv.noConfusion` is the first uncached declaration after a fast reused prefix, and it is a generated definition over heterogeneous equality and many structure fields.  The v4 retry confirms that the boundary is independent of the old cache format and the earlier low-disk host condition.  This result is different from the Omega and rational proof walls: the hard object is derived support generated for a structure, so any shortcut belongs outside the MPC typing rules unless the project explicitly adds a derived checker for this class of declarations.
