...
| Abelian image-zero lemma | `Mathlib.CategoryTheory.Abelian.Basic` | `CategoryTheory.Abelian.image_ι_comp_eq_zero` | Rejected: performance | The first attempt used the unqualified root `CategoryTheory.image_ι_comp_eq_zero`, and the driver rejected the metadata-only export.  The corrected root built and exported a 27 MB artifact, but `mpc-check-export` was killed with exit code 137 before producing checker output.  Disk space was available, so the result is a host-resource boundary rather than an MPC semantic rejection. |
...
The first abelian-category probe reached a host-resource boundary rather than a checker rule boundary.  The unqualified root `CategoryTheory.image_ι_comp_eq_zero` produced only exporter diagnostics and metadata, and the probe driver rejected that artifact before invoking MPC.  The corrected root `CategoryTheory.Abelian.image_ι_comp_eq_zero` built successfully and exported a 27 MB artifact, but `mpc-check-export` was killed with exit code 137 before producing declaration stats or a semantic rejection.
