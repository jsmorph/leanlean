...
The next two abelian batches accepted.  The first added coimage/image comparison facts and the epi/mono factorization API through `monoLift_comp`; the migrated cache already contained all 5,552 target declarations by the time of the recorded run.  The second added the pullback and pushout stability lemmas for epimorphisms and monomorphisms, checking 185 new declarations around the biproduct presentation of pullbacks and pushouts.  These runs widen the abelian-category coverage without exposing a new rule package.
