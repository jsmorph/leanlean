...
An inductive declaration introduces exactly one inductive type in the first subset.  The declaration may bind parameters, but it may not bind indices.  Each constructor field list is a telescope: the first field type is checked under the parameters, and each later field type is checked under the parameters and the earlier fields.  Constructor result targets remain the inductive type applied to its parameters, so constructor results may not vary by field-dependent indices.
...
| Indices | Omitted | Indexed constructor targets require a more general inductive-family specification. |
