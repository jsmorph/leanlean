...
Status: partially implemented.  Environment entries now use a single declaration record with kind metadata, optional checked values, primitive recursor metadata, and transparent versus opaque definition behavior.  Projection support is specified for the first non-indexed structure fragment.  Theorem metadata and reducibility hints beyond opacity remain outside the current subset.
