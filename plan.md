...
Status: partially implemented.  Environment entries now use a single declaration record with kind metadata, optional checked values, primitive recursor metadata, projection metadata, theorem metadata, and transparent versus opaque definition behavior.  Ordered declaration scripts fold over the checked admission APIs, giving future export parsing a single kernel-facing target.  Kernel-style inductive declarations can enter as type-former and constructor types with a parameter count, and the adapter reconstructs the local inductive block before running the ordinary inductive checker.  Projection support is implemented for one-constructor inductives, including indexed projection maps, dependent projection result types, and eta for non-recursive data structures whose constructor target matches the major premise type.  Theorem declarations are checked proof declarations whose types must live in `Prop`, whose proof values are stored, and whose constants do not unfold during conversion.  Reducibility hints beyond opacity and inherited structure metadata remain outside the current subset.
