...
Ordinary universe polymorphism now covers inference, conversion, axioms, definitions, inductive declarations, generated constructors, and generated recursors.  The kernel reserves `Sort 0` for `Prop`, uses symbolic `imax` for dependent function sorts, applies proof irrelevance to terms with the same normalized proposition type, and supports proposition-valued inductives with Prop-only elimination.  The next implementation work should specify Lean's large-elimination exceptions for propositions before quotient primitives expand the trusted rules.
