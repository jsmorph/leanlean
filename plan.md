...
The next priority is `leanlean-check-export`.  The module checker already gives immediate feedback on compiled Lean modules, but it still relies on Lean's module loader and environment data.  The export checker is the first independent artifact checker: it must read a `lean4export` NDJSON file, translate the records into declaration scripts, and replay those scripts through the local kernel without asking Lean to interpret the artifact.
...
| Exact conversion equivalence | Conversion is normalization-based with beta, delta, zeta, iota, projection, natural-literal, quotient, eta, and proof-irrelevance rules for the specified fragment. | Broad Lean exports stress transparency, reducibility metadata, universe arithmetic, proof irrelevance, nested recursor computation, quotient computation, and eta in combinations that named tests do not cover. | Expand adversarial conversion tests before adding new features.  State and then discharge preservation obligations for every reduction rule, and compare local normal forms against Lean over larger exported fragments. |
...
| Structured name fidelity | Internal names are strings, translated from Lean names by `toString`. | Export files carry structured `Name` records.  String encoding may be sufficient over a controlled fragment, but a serious checker should not depend on an unstated injectivity assumption. | Preserve Lean-style structured names in the syntax, or specify and test an injective encoding for all accepted export names.  Add collision tests before relying on string names for adversarial export input. |
| Full recursor faithfulness | The generator covers the tested mutual, nested, indexed, proposition-valued, quotient-adjacent, and sort-polymorphic subsingleton cases, and generated recursor metadata is checked against regenerated declarations. | Recursors are the highest-risk generated kernel objects.  Exports from broader libraries may reveal target ordering, helper naming, motive ordering, or rule RHS cases not yet covered. | Use export-checker tests to replay recursor-heavy declarations first.  Add each mismatch as either a Lean divergence with a reason or a generator correction with a regression. |
