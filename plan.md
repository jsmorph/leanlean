...
The remaining work should run in this order.  First, replay should reject unsafe declarations by default, with any future unsafe mode kept outside the trusted-kernel claim.  Second, literal expressions should enter the syntax, importer, type checker, normalizer, specification, and regression suite.  Third, the importer should translate Lean's full structure-extension records and check inherited projections against exported metadata.  Fourth, recursive-definition compilation artifacts should either replay through a written kernel-facing rule or fail with a documented rejection.  Broad environment import should expand only after those representation questions have written rules and tests.

Acceptance criteria:

- Transparent and opaque constants behave differently under conversion.
- Trusted replay rejects unsafe declarations by default unless replay runs in an explicitly named mode with a separate trust claim.
