...
The first specification draft lives in `spec.md`.  It now covers the data fragment with universe-polymorphic axioms, transparent and opaque definitions, inductive blocks, parameters, indices, dependent constructor-field telescopes, strictly positive recursive fields, generated recursor families, low-level quotient primitives, and beta, delta, zeta, iota, and quotient reduction.  It includes `Prop`, proof irrelevance, indexed subsingleton elimination, and mutual inductive blocks with shared parameters.  It still omits the higher-level `Quotient` API, projections, theorem metadata, and the full declaration model.
