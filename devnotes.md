...
The importer also has a root-name closure extractor for Lean environments.  The extractor follows constants mentioned in declaration types, definition or theorem values, opaque values, inductive `all` and constructor lists, constructor parent metadata, and recursor metadata, and then sends the collected `ConstantInfo` values through snapshot replay.  This step separates dependency discovery from declaration admission; the final gate remains the local replay checker.
...
The widened smoke test now imports ordinary definitions, opaque definitions, propositions, mutual inductives, dependent structures with projections, equality, quotients, `Nat`, `Bool`, `List`, and indexed local inductives from real Lean environment data.  Nested mutual roots such as `MutNestA` still fail generated recursor replay.  The current local recursor shape exposes nested helper targets as extra motives and minors, while Lean's exported recursor shape for the accepted nested mutual example differs; this is a recorded faithfulness gap, not an accepted divergence.
