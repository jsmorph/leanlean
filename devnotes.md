...
- [ ] Extend universe polymorphism to inductive declarations and generated recursors.

## Current Decisions

The first specification draft lives in `spec.md`.  It now covers the data fragment with universe-polymorphic axioms and definitions, single inductive declarations, parameters, indices, dependent constructor-field telescopes, strictly positive recursive fields, generated recursor families, and beta, delta, zeta, and iota reduction.  It still omits `Prop`, proof irrelevance, quotient types, mutual inductives, and universe-polymorphic inductive declarations.

Universe levels are represented by a small level language rather than by raw natural numbers.  Axioms and definitions may declare universe parameters, and the inferencer, normalizer, and conversion checker now carry an explicit universe context.  Inductive result universes remain explicit and closed, and the kernel checks that constructor arguments do not force a larger universe.  The term language carries explicit universe arguments on constants, because primitive recursors need a motive universe parameter in order to exist as ordinary constants with ordinary types.  The kernel now enforces that these universe arguments are arity-correct and closed under the active universe context in both inference and normalization, rather than trusting normalization to see only well-formed constants.
...
Universe-polymorphic definitions now have a checked path through the kernel.  `addDefinitionWithLevels` and `addAxiomWithLevels` reject duplicate universe parameters and reject level variables outside the declared universe context.  The raw public entry points still reject open universe levels.  The example `polyId.{u}` instantiates at both `Type 0` and `Type 1`, and the focused regression tests cover symbolic level ordering and universe-context closure.
