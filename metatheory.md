...
Equality primitives require a preservation theorem for `Eq.rec` and `Eq.ndrec`.  A redex whose proof reduces to `Eq.refl` returns the minor premise at the reflexive endpoint, and the K-style endpoint case requires the endpoints to reduce to alpha-equivalent terms before the same reduction applies.  The proof obligation is to show that the motive instantiation after reduction matches the declared result type.
