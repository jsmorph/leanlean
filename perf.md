...
The first in-memory memoization experiment was negative.  An exact structural cache for WHNF and successful defEq moved work into key hashing and equality over large expressions; the selected declaration stopped between profiler heartbeats.  A bounded variant, enabled only with `mpc-dynamic-profile-export --memo`, caches WHNF results for expressions with at most 128 nodes and caches successful defEq only for tiny empty-context pairs.  On the 10,000-step run, bounded WHNF caching produced 4,567 WHNF hits, reduced counted WHNF calls from 3,975 to 1,064, and reduced transparent unfolds from 270 to 12, but it did not make the declaration complete.  On the 100,000-step run, the memoized version reached only the 11,000-step heartbeat after about 90 seconds inside the declaration, worse than the non-memoized diagnostic, which reached the 23,000-step heartbeat in the same window.  This rejects small exact-key memoization as the next fix for this wall.

Later diagnostics changed the classification.  The dynamic profiler lacked the checker's top-level alpha-equivalence fast path and did not reduce ordinary recursors.  The earlier profile therefore overstated structural-conversion cost and later produced a profiler-only rejection at `LinearEquiv.rec`.  Once the profiler matched the checker, the selected declaration exposed a universe-level issue: repeated `imax 0 _` layers and Prop checks against zero made equivalent levels miss in `Level.defEq`.

The checker fix sits inside the universe package.  `Level.defEqZero` recognizes levels definitionally equal to zero without normalizing the whole level, `inferPiSort` and proposition checks use that predicate, and `Level.reduceIMax` removes `imax` when the reduced left side is zero.  `Expr.liftFrom` now returns immediately for zero lifts, which avoids traversing a substituted argument when the operation is the identity.  With these changes, `mpc-dynamic-profile-export --declaration 7358 --budget 100000` checks `LinearEquiv.noConfusion` in 31,167 counted steps and 33 ms after cached prefix replay, and a 600-second cached stats run passes the old index-7358 wall and reaches index 12,736 before timeout.

The next determinant boundary is `_private.Mathlib.LinearAlgebra.InvariantBasisNumber.0.inducedEquiv._proof_4` at index 12,736.  A 30-minute warm-cache replay reached that declaration after reusing the prefix, produced no stderr, and timed out with no row after the started event.  A focused diagnostic initially rejected there, but the rejection came from profiler drift: the dynamic WHNF path still counted primitive and projection attempts without invoking the checker reducers.

After the profiler began reducing primitive-Nat and projection redexes through the same reducer APIs as the checker, the focused declaration no longer rejected.  The 1,000,000-step corrected profile exhausted its budget in 817 ms of selected-declaration time after cached prefix replay, with 342,449 WHNF calls, 85,131 defEq calls, 97,430 successful projection reductions, and repeated unfolds of `Pi.instMul`, `Pi.instZero`, `Pi.instAdd`, `Pi.nonUnitalNonAssocSemiring`, `Pi.nonUnitalSemiring`, and `Pi.semiring`.  The bounded WHNF memo path cut WHNF calls to 14,522 in the same step budget, but defEq calls rose to 367,328 and the declaration still did not complete.

Three further experiments were negative and were not kept.  Allowing tiny defEq memoization in non-empty contexts produced 35,011 defEq-cache hits but increased selected-declaration time to 3,850 ms and still exhausted the 1,000,000-step budget.  Constructor-field proof irrelevance and theorem-headed proof irrelevance both moved work into type inference and proposition checking, with about 170,000 to 186,000 infer calls in the same budget, so the next useful optimization should target repeated algebra-instance projection comparison without broad proof-type probing.

The shape-telemetry run makes that target concrete.  A 10,000,000-step focused profile exhausted its budget with 876,144 defEq calls, 3,464,215 WHNF calls, and 926,546 successful projection reductions.  The top post-WHNF structural comparisons are inherited-structure projection chains, including `proj CommRing.0`, `proj Ring.0`, `proj Semiring.0`, and `proj NonUnitalSemiring.0`, often comparing terms whose target sizes differ.  The top context depths are 9 through 18, so a generic closed-term cache would miss the main workload.  The next experiment should inspect whether the repeated algebra-instance parent projections have a principled normalization or comparison rule already present in Lean's representation.

The useful fix was to preserve projection-target WHNF when a projection cannot reduce to a constructor field.  Projection reduction already computed the target WHNF, so this change keeps information that had been thrown away and normalizes equal inherited-class paths to the same projection chain under the source instance.  On declaration 12,736, the 1,000,000-step profile changed from 608 ms, 85,131 defEq calls, and 342,449 WHNF calls to 338 ms, 43,688 defEq calls, and 412,235 WHNF calls.  The 10,000,000-step profile changed from 6,423 ms and 876,144 defEq calls to 3,147 ms and 444,324 defEq calls, but it still exhausted the budget.  Retesting bounded memoization after this fix was negative: the 1,000,000-step `--memo` run took 1,514 ms and raised defEq calls to 377,945 despite 294,433 WHNF-cache hits.

Two further local experiments were rejected at this point.  A post-WHNF alpha-equivalence check removed some identical projection chains after the target-WHNF fix, but selected-declaration time stayed flat at the 1,000,000-step budget.  Constructor-field proof irrelevance and theorem-headed proof irrelevance moved work into type inference and proposition checking, so further proof-field work needed cached or expected-type information at the comparison site.
...
Constructor proof-field conversion is useful as a rule, but it does not solve declaration 12,736.  MPC now records, in the runtime environment side table, which fully applied constructor fields have proposition-valued types, and structural conversion skips those fields when `Prop` is enabled and both sides apply the same constructor at full arity.  The final 1,000,000-step profile recorded 2,892 skipped proof-field comparisons, reduced defEq calls from 43,688 to 32,872, changed WHNF calls from 412,235 to 423,290, and changed selected-declaration time from about 338 ms to 323 ms.

The 10,000,000-step focused profile shows the same shape.  It recorded 26,577 skipped proof-field comparisons, reduced defEq calls from 444,324 to 324,959, changed WHNF calls from 4,142,215 to 4,253,917, and changed selected-declaration time from 3,147 ms to 3,334 ms.  This keeps the optimization because it applies proof irrelevance at a known constructor field rather than inferring arbitrary proof types during conversion, but the determinant wall remains a WHNF and projection loop through pointwise algebra-instance terms.

Repeat diagnostics changed the interpretation of post-WHNF alpha comparison.  On declaration 12,736, almost every repeated non-alpha `defEq` goal succeeds, so a post-WHNF alpha fast path targets successful work rather than failed fallback probing.  Retesting after constructor proof-field conversion reduced the 10,000,000-step profile from 324,959 to 272,965 defEq calls and recorded 41,678 post-WHNF alpha hits.  WHNF calls rose from 4,253,917 to 4,301,135, and selected-declaration time changed from 3,334 ms to 3,295 ms.  This is a small win at best, but the rule is simple and preserves conversion.

The success-only defEq cache experiment is the first profile-only optimization that checks declaration 12,736 within the selected budget.  It caches only successful non-alpha `defEq` goals and does not cache failures or WHNF results.  Three 1,000,000-budget runs checked the declaration in 716,661 counted steps, with 4,232 success-cache hits, 5,694 misses, 5,679 entries, and selected-declaration times of 831 ms, 784 ms, and 766 ms.  This directly validates the repeat-outcome finding.

The adapter-level replay experiment tested that cache in the full determinant artifact.  `mpc-check-export --cache-layer .tmp/mathlib-probes/mathlib-cache-v4.db --stats-jsonl --defeq-success-cache .tmp/mathlib-probes/linearMap-det-comp.ndjson` accepted the artifact, reusing 12,736 declaration entries and checking 56 entries.  Declaration 12,736 checked in 890 ms; the full run reported 192,168 ms cumulative replay time, and the slowest checked declaration was `Matrix.toLin'_mul_apply` at 2,092 ms.  The measured checked suffix totaled 11,151 ms, so the old wall was repeated conversion inside one theorem rather than a sequence of comparable blockers.
