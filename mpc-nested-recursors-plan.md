...
The local-telescope slice now handles recursive fields whose nested occurrence appears under function binders.  The generated induction hypothesis preserves the field-local binder telescope, and iota reduction constructs the matching lambda by applying the recursive field to those local arguments before calling the helper recursor.  This covers shapes such as `(n : Nat) -> List T`; indexed helper targets whose indices depend on those locals remain a separate schema-target extension.
