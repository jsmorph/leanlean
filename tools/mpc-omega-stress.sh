...
profile_dir="${LEANLEAN_PROFILE_DIR:-.tmp}"
timeout_seconds="${LEANLEAN_MPC_STRESS_TIMEOUT:-120}"
...
lake build Faithfulness.ExportOmega mpc-check-export
...
LEAN_PATH="$lean_path" "$lean4export_bin" \
  Faithfulness.ExportOmega \
  -- \
  LeanLeanFaithfulness.ExportOmega.nat_linear_bounds \
  > "$artifact"
